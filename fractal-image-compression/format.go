...
// This file is from gonum/mat/format.go, with the addition of min() and max() functions
package main
...
func Formatted(m mat.Matrix, options ...FormatOption) fmt.Formatter {
	f := formatter{
		matrix: m,
		dot:    '.',
	}
...
type formatter struct {
	...
	format func(m mat.Matrix, prefix string, margin int, dot byte, squeeze bool, fs fmt.State, c rune)
}
...
func FormatCSV() FormatOption {
	return func(f *formatter) { f.format = formatCSV }
}
...
func formatCSV(m mat.Matrix, prefix string, _ int, _ byte, squeeze bool, fs fmt.State, c rune) {
	rows, cols := m.Dims()
...
func format(m mat.Matrix, prefix string, margin int, dot byte, squeeze bool, fs fmt.State, c rune) {
	rows, cols := m.Dims()
...
func formatMATLAB(m mat.Matrix, prefix string, _ int, _ byte, squeeze bool, fs fmt.State, c rune) {
	rows, cols := m.Dims()
...
func formatPython(m mat.Matrix, prefix string, _ int, _ byte, squeeze bool, fs fmt.State, c rune) {
	rows, cols := m.Dims()
...
func maxCellWidth(m mat.Matrix, c rune, printed, prec int, w widther) ([]byte, int) {
	var (
		buf        = make([]byte, 0, 64)
		rows, cols = m.Dims()
		max        int
